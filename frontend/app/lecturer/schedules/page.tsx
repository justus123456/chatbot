...
import { LecturerScheduleWorkflow } from "@/components/governance/lecturer-workflows";
...
export default function LecturerSchedulesPage() {
  ...
  if (!user || !canAccessLecturer(user.role)) return <GovernanceRestricted message="Only lecturer accounts can open lecturer schedules." />;

  return (
    <GovernanceShell role="lecturer" user={user}>
      <GovernancePageFrame>
        <LecturerScheduleWorkflow user={user} />
      </GovernancePageFrame>
    </GovernanceShell>
  );
}
