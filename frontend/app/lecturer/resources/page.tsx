...
import { LecturerResourcesWorkflow } from "@/components/governance/lecturer-workflows";
...
export default function LecturerResourcesPage() {
  ...
  if (!user || !canAccessLecturer(user.role)) return <GovernanceRestricted message="Only lecturer accounts can manage lecturer resources." />;

  return (
    <GovernanceShell role="lecturer" user={user}>
      <GovernancePageFrame>
        <LecturerResourcesWorkflow user={user} />
      </GovernancePageFrame>
    </GovernanceShell>
  );
}
