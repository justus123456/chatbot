...
import { LecturerEscalationsWorkflow } from "@/components/governance/lecturer-workflows";
...
export default function LecturerEscalationsPage() {
  ...
  if (!user || !canAccessLecturer(user.role)) return <GovernanceRestricted message="Only lecturer accounts can answer lecturer escalations." />;

  return (
    <GovernanceShell role="lecturer" user={user}>
      <GovernancePageFrame>
        <LecturerEscalationsWorkflow />
      </GovernancePageFrame>
    </GovernanceShell>
  );
}
