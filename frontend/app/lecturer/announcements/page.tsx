...
import { LecturerAnnouncementsWorkflow } from "@/components/governance/lecturer-workflows";
...
export default function LecturerAnnouncementsPage() {
  ...
  if (!user || !canAccessLecturer(user.role)) return <GovernanceRestricted message="Only lecturer accounts can manage lecturer announcements." />;

  return (
    <GovernanceShell role="lecturer" user={user}>
      <GovernancePageFrame>
        <LecturerAnnouncementsWorkflow user={user} />
      </GovernancePageFrame>
    </GovernanceShell>
  );
}
